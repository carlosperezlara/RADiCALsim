...
namespace {
  void PrintUsage() {
    G4cerr << " Usage: " << G4endl;
    G4cerr << " exampleRAD [-m macro ] [-u UIsession] [-t nThreads] [-vDefault]"
           << G4endl;
...
int main(int argc,char** argv) {
  ...
  auto detConstruction = new RAD::DetectorConstruction();
  runManager->SetUserInitialization(detConstruction);

  auto physicsList = new FTFP_BERT;
  runManager->SetUserInitialization(physicsList);

  auto actionInitialization = new RAD::ActionInitialization();
